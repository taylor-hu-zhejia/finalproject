<template>
  <div class="game-page">
      ...
      <!-- <div class="game-result" v-if="gameResult">
       ...
       <div class="spinner">
        <div class="spinner-grid">
          <div
          class="wheel-element"
          v-for="(element, index) in wheelElements.slice(0, 4)"
         :key="index"
         :class="{ 'color-change': activeIndex === index }">
         {{ element }}
        </div>
        <div
        class="wheel-element"
        v-for="(element, index) in wheelElements.slice(4, 5)"
        :key="index"
        :class="{ 'color-change': activeIndex === index + 4 }"
        >
      {{ element }}
    </div>
    <div class="controls">
    ...
    <div
      class="wheel-element"
      v-for="(element, index) in wheelElements.slice(5, 6)"
      :key="index"
      :class="{ 'color-change': activeIndex === index + 5 }"
    >
      {{ element }}
    </div>
    ...
    <div
      class="wheel-element"
      v-for="(element, index) in wheelElements.slice(6, 10)"
      :key="index"
      :class="{ 'color-change': activeIndex === index + 6 }"
    >
      {{ element }}
    </div>
  </div>
...
`This game design not only caters to the entertainment needs of users but also ensures the security and transparency of funds, providing a fun and relatively safe environment for users to enjoy.`
    ...
    const connectWallet = async () => {
      if (window.ethereum) {
        try {
          ...
          const start=accounts[0].substring(0,5);
          const end=accounts[0].substring(accounts[0].length - 3);
          currentAccount.value = `${start}...${end}`;
    ...
    const contractAddress = '0x394Ba8dDf74BD27764Ebe9736f6f6A083e9Af0E6'; 
  ...
  try {
    ...
    console.log(result);
    stop();
    // Handle transaction result logs
    if (result.events && result.events.playEvent) {
      const playEvent = result.events.playEvent;
      const topics = playEvent.raw.topics;

      const num1 = parseInt(topics[2], 16); 
      const num2 = parseInt(topics[3], 16); 
      alert(`You won ${num1} game tokens, multiplier: ${num2}`) ;
    }
  } catch (error) {
...
const spinWheel=()=> {
      ...
      const spin = () => {
        ...
        index = (index + 1) % wheelElements.value.length;
        setTimeout(spin, 200); 
      };
      spin();
    };
    ...
    const listenToPlayEvent = () => {
      ...
      .on('data', (event) => {
        ...
        try {
          ...
          const start = formattedDate.substring(0, 5);
          const end = formattedDate.substring(formattedDate.length - 3);
          ...
          const eventMessage = ` ${start}...${end}  \t ${address} \t won ${num1} game token,multiplier: ${num2}`;
  ...
  try {
    ...
    const audio = new Audio('/7586.wav'); 
    audio.play();
    closeDepositModal(); 
    const balanceValue = await contract.value.methods.getG2TokenBalance().call({ from: currentAccount.value });
    balance.value = balanceValue;
